...
export function ButtonGroup(prop) {
  return <div className={classNames('hbw-btn-group', prop.className)}>{prop.children}</div>;
}
...
export function Button(prop) {
  ...
  const depth = prop.depth ? `depth-${prop.depth}` : '';
  // const
  return (
    <button
      type={prop.htmlType}
      ...
      onClick={prop.to ? () => prop.history.push(prop.to) : prop.onClick}
    >
